...
В качестве отказоустойчивого решения реализация включает Outbox-модуль.
Под него была создана отдельная таблица outbox_events.
...
- [3. Запуск тестов](#3-запуск-тестов)
  ...
  - [3.3. Нагрузочный тест](#33-нагрузочный-тест)
...
Перед запуском контейнера важно под текущую потребность настроить воркеров.
...
### 3.3 Нагрузочный тест
-n - общее количество задач
-c - максимальное количество одновременных запросов
```bash
python load_test.py --n 5000 --c 50
```
